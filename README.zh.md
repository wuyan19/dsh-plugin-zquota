...
- **数据归属**：账号与 API Key 存于本机 `$DSH_HOME/.credentials.yaml`（`ZAI_QUOTA_ACCOUNTS` 索引 + `ZAI_QUOTA_KEY_<id>`），API Key 明文永不进入浏览器
...
| 半侧 | 入口 | 职责 |
|---|---|---|
| Host | `src/host.js`（`exports "."`，纯 ESM 零构建） | 凭据存储、curl 配额查询、`webServer` 前缀路由 `/zquota-api` 提供 JSON API |
...
- API Key 存于 `~/.dsh/.credentials.yaml`（0600），与 DSH 自身凭据同库同权限等级
