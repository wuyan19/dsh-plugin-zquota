	...
	factory: (require) => {
		...
		*/
		...
		function GlmSection(props) {
			...
			const provisionLine = provisionNotes.notes.length > 0 ? h("p", { className: "gq-infoline" }, t("provision.done", { list: provisionNotes.notes.map((n) => t("provision." + n, { route: provisionNotes.route || "zai-coding-cn" })).join(t("provision.sep")) })) : null;
			...
			}, t("action.add")))), err ? h("p", { className: "gq-err" }, err) : null, liveHint, provisionLine, editor ? renderEditor() : null, data === null ? h("p", { className: "gq-ts" }, t("loading")) : accounts.length === 0 ? h("div", { className: "gq-empty" }, h("div", null, t("empty.title")), h("button", {
				className: "gq-btn gq-primary",
				onClick: () => openEditor(null)
