		...
		*/
		...
		function MeterCell(label, item) {
			if (!item) return h("div", {
				className: "gq-cell",
				key: label
			}, h("div", { className: "gq-cell-lbl" }, label), h("div", { className: "gq-cell-val" }, "—"), h("div", { className: "gq-track" }), h("div", { className: "gq-cell-sub" }, "\xA0"));
			...
			let title = null;
			if (Array.isArray(item.usageDetails) && item.usageDetails.length > 0) title = "各工具用量：\n" + item.usageDetails.map((r) => r.modelCode + "：" + r.usage).join("\n");
			const parts = [];
			...
			return h("div", {
				...
				key: label,
				title: title || void 0
			}, h("div", { className: "gq-cell-lbl" }, label), h("div", { className: "gq-cell-val" }, pct == null ? "—" : String(pct), h("span", { className: "gq-unit" }, "%")), h("div", { className: "gq-track" }, h("span", {
				className: "gq-fill",
				style: { width: width + "%" }
			})), h("div", { className: "gq-cell-sub" }, parts.length > 0 ? parts.join(" · ") : "\xA0"));
		}
		function GlmSection(props) {
			...
			react.useEffect(() => clientCtx.interval(() => setTick((t) => t + 1), 6e4), []);
			...
			function renderRow(a) {
				...
				return h("li", {
					...
					key: a.id
				}, h("div", { className: "gq-cardhead" }, h("span", { className: "gq-name" }, a.name), h("span", { className: "gq-tag" }, a.endpoint === "intl" ? "INTL" : "CN"), levelLabel ? h("span", { className: "gq-tag" }, levelLabel) : null, a.live ? h("span", { className: "gq-live" }, "使用中") : null, h("span", { className: "gq-cardactions" }, a.live ? null : h("button", {
				...
				}, confirming ? "✓" : "✕"))), r || a.lastError ? h("div", { className: "gq-meters" }, MeterCell("5 小时", r && r.fiveHour), MeterCell("每周", r && r.weekly), MeterCell("MCP 月", r && r.monthlyMcp)) : null, a.lastError ? h("p", { className: "gq-err" }, a.lastError) : null, h("div", { className: "gq-foot" }, h("span", { className: "gq-ts" }, a.lastUpdated ? "更新于 " + timeAgo(a.lastUpdated) : "未刷新")));
			}
			function renderEditor() {
				return h("div", {
					className: "gq-editor",
					key: "editor"
				}, h("div", { className: "gq-field" }, h("span", { className: "gq-fieldlabel" }, "名称"), h("input", {
					className: "gq-input",
					value: editor.name,
					placeholder: "例如：工作账号",
					autoComplete: "off",
					onChange: (e) => setEditor(Object.assign({}, editor, { name: e.target.value }))
				})), h("div", { className: "gq-field" }, h("span", { className: "gq-fieldlabel" }, "API Key"), h("input", {
				...
				})), h("div", { className: "gq-field" }, h("span", { className: "gq-fieldlabel" }, "版本"), h("div", { className: "gq-seg" }, h("button", {
					className: editor.endpoint === "cn" ? "active" : "",
					onClick: () => setEditor(Object.assign({}, editor, { endpoint: "cn" }))
				}, "国内版 open.bigmodel.cn"), h("button", {
					className: editor.endpoint === "intl" ? "active" : "",
					onClick: () => setEditor(Object.assign({}, editor, { endpoint: "intl" }))
				}, "国际版 api.z.ai"))), edErr ? h("p", { className: "gq-err" }, edErr) : null, h("div", { className: "gq-editoractions" }, h("button", {
			...
			let liveHint = null;
			if (liveInfo && liveInfo.source === "env") liveHint = h("p", { className: "gq-warnline" }, "当前凭据来自进程环境变量（ZAI_CODING_CN_API_KEY），在面板内切换不会生效。");
			else if (liveInfo && !liveInfo.configured) liveHint = h("p", { className: "gq-warnline" }, "尚未配置 ZAI_CODING_CN_API_KEY，请选择一个账号点「设为当前」。");
			return h("div", { className: "gq-section" }, h("div", { className: "gq-head" }, h("div", null, h("h3", { className: "gq-title" }, "GLM 编程套餐"), h("p", { className: "gq-sub" }, "智谱 Coding Plan 用量监控 · 「设为当前」切换账号，下一次请求即生效")), h("div", { className: "gq-headactions" }, h("button", {
				className: "gq-btn gq-secondary",
				disabled: busyAll || !data || accounts.length === 0,
				onClick: refreshAll
			}, busyAll ? "刷新中…" : "刷新全部"), h("button", {
				className: "gq-btn gq-primary",
				onClick: () => openEditor(null)
			}, "添加账号"))), err ? h("p", { className: "gq-err" }, err) : null, liveHint, editor ? renderEditor() : null, data === null ? h("p", { className: "gq-ts" }, "加载中…") : accounts.length === 0 ? h("div", { className: "gq-empty" }, h("div", null, "还没有账号"), h("button", {
				className: "gq-btn gq-primary",
				onClick: () => openEditor(null)
			}, "添加账号")) : h("ul", { className: "gq-rows" }, accounts.map(renderRow)));
		}
		function apply(ctx) {
			clientCtx = ctx;
