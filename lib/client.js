		...
		function MeterCell(label, item) {
			...
			if (item.nextResetTime) {
				...
				subChildren.push(h("span", {
					className: "gq-reset",
					title: "重置于 " + new Date(item.nextResetTime).toLocaleString()
		...
		function GlmSection(props) {
			...
			function renderRow(a, idx, arr) {
				...
				return h("li", {
					...
					key: a.id
				}, h("div", { className: "gq-cardhead" }, h("span", { className: "gq-name" }, a.name), h("span", { className: "gq-tag" }, a.endpoint === "intl" ? "INTL" : "CN"), levelLabel ? h("span", { className: "gq-tag" }, levelLabel) : null, a.live ? h("span", { className: "gq-live" }, "使用中") : null, h("span", { className: "gq-cardactions" }, h("button", {
					className: "gq-icon",
					disabled: busy || idx === 0,
					title: "上移",
